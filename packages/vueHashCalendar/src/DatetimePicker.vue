        ...
        <div class="calendar_content" :style="{'height': `${calendarContentHeight}px`}" @click.stop>
            <div class="calendar_title" ref="calendarTitle">
                <div class="calendar_title_date">
                  <span @click="prev"><svg t="1585382751715" class="icon" style='width:20px;display:block;transform: rotate(180deg)' viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2848" ><path d="M347.687 144.188l-52.761 52.238 313.928 316.082-316.568 313.42 52.314 52.673 369.322-365.663z" p-id="2849" fill="#898989"></path></svg></span>
                    <span v-if="pickerType !== 'time'" class="calendar_title_date_year"
                          :class="{'calendar_title_date_active': isShowCalendar}"
                          @click="showCalendar">{{ `${checkedDate.year}年${this.checkedDate.month + 1}月` }}</span>
                  <span @click="next"><svg t="1585382751715" class="icon" style='width:20px;display:block' viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2848" ><path d="M347.687 144.188l-52.761 52.238 313.928 316.082-316.568 313.42 52.314 52.673 369.322-365.663z" p-id="2849" fill="#898989"></path></svg></span>
                </div>
            ...
            <calendar ref="calendar" v-if="pickerType !== 'time'"  @buqian="buqian" @daka="daka" :status="status" :show="isShowCalendar" v-bind="{...$props, ...$attrs}" @height="heightChange"
                      :default-date="defaultDatetime" @touchstart="touchStart" @touchmove="touchMove" @touchend="touchEnd" @slidechange="slideChange"
                      @change="dateChange" @click="dateClick" @prev="prev" @next="next"></calendar>
